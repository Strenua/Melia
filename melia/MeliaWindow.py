...
class MeliaWindow(Window):
    ...
    def start_panel(self):
        ...
        self.panel.ui.dashbutton.connect('enter-notify-event', self.show_dash)
        if preferences['panel_transparent']: self.set_transparent()
    ...
    def get_button_position(self, button):
        ...
        try: by = ((self.btns.index(button) * bsry) + (4 * self.btns.index(button))) + wy
        except ValueError: # assume its the trash button...
            by = self.get_screen().get_height()
        return bx, by
