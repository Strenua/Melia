...
name = "battery-status"
comment = "Battery Status applet for the GNOME desktop"
homepage = "http://live.gnome.org/BatteryStatus"
copyright = "Copyright © 2010 Ivan Zorin"
...
try:
	...
	class button(indicator.Indicator):
		...
		# bool var. of suspend support for sleep actions
		can_suspend = True
		# bool var. of hibernate support for sleep actions
		can_hibernate = True
		# power source device
		power_dev = ""
		...
		def init_widgets(self):
			"""Create widgets for indicator"""
			# init battery indicator state
			...
			# Powersave item for Power Mode
			self.item_mode_powersave = gtk.RadioMenuItem(None, "Powe_rsave")
			# Ondemand item for Power Mode
			self.item_mode_ondemand = gtk.RadioMenuItem(self.item_mode_powersave , "On_demand")
			# Conservative item for Power Mode
			self.item_mode_conservative = gtk.RadioMenuItem(self.item_mode_powersave , "N_ormal")
			# Performance item for Power Mode
			self.item_mode_performance = gtk.RadioMenuItem(self.item_mode_powersave , "P_erformance")
			# Power Modes items separator
		...
		def init_first_run(self):
			"""After-init hook"""
			# if user run indicator at first time and use gnome-power-manager icon in indicator area
			if self.option_firstrun and self.option_gpm_icon_policy != "never":
				# then show advice to remove icon from indicator area
				message_icon_tray = gtk.MessageDialog(parent = None, type = gtk.MESSAGE_QUESTION, buttons = gtk.BUTTONS_NONE, message_format = None)
				title_text = "Battery Status indicator has been added in Indicator area."
				message_text = 'Battery Status indicator shows state of battery in your laptop, but you also use Power Manager\'s battery icon for the same purpose - would you like to remove it?\n\nYou can always change Power Manager\'s battery icon settings and its behavior in "Power Management" Preferences'
		...
		def init_gconf_settings(self):
			"""Getting indicator settings from GConf"""
		...
		def init_gconf_changes(self, client, connection_id, entry, option):
			...
			### update GConf settings on the fly, if it's changing within indicator works
			# update icon policy option
			if option == "icon" and entry.get_value().type == gconf.VALUE_STRING:
		...
		def update_main_menu(self, item):
			...
			# update sleep items
			if item == "sleep":
				if self.option_sleep and self.can_suspend and self.can_hibernate and not self.power_error:
					self.item_sleep_separator.show()
					self.item_suspend.show()
					self.item_hibernate.show()
				else:
					self.item_sleep_separator.hide()
					self.item_suspend.hide()
					self.item_hibernate.hide()
			# update session items
			elif item == "session":
				if self.option_session and self.package_session:
					self.item_session_separator.show()
					self.item_logout.show()
					self.item_shutdown.show()
				else:
					self.item_session_separator.hide()
					self.item_logout.hide()
					self.item_shutdown.hide()
			# update power items
			elif item == "power":
				if self.option_power and self.package_power:
					self.item_power.set_sensitive(True)
					self.item_pmsettings.show()
					self.power_separator.show()
				else:
					self.item_power.set_sensitive(False)
					self.item_pmsettings.hide()
					self.power_separator.hide()
			# update CPU frequency scaling items
			elif item == "cpufreq":
				if self.option_cpufreq and self.package_cpufreq and not self.cpufreq_error:
					self.item_mode_powersave.show()
					self.item_mode_ondemand.show()
					self.item_mode_conservative.show()
					self.item_mode_performance.show()
					# detect current CPU frequency scaling mode
					if self.powermode == "powersave":
						self.item_mode_powersave.set_property("active", True)
					elif self.powermode == "ondemand":
						self.item_mode_ondemand.set_property("active", True)
					elif self.powermode == "conservative":
						self.item_mode_conservative.set_property("active", True)
					elif self.powermode == "performance":
						self.item_mode_performance.set_property("active", True)
				else:
					self.item_mode_powersave.hide()
					self.item_mode_ondemand.hide()
					self.item_mode_conservative.hide()
					self.item_mode_performance.hide()
			else:
				pass
		...
		def init_main_menu(self):
			...
			self.item_pmsettings = gtk.MenuItem("_Power Management...", True)
			self.item_pmsettings.connect("activate", self.main_menu_action)
			self.item_pmsettings.show()
			# menu item: Battery Status
			self.item_status = gtk.MenuItem()
			self.item_status.set_label("Battery Status")
			self.item_status.set_sensitive(True)
			self.item_status.connect("activate", self.main_menu_battery_dialog)
			self.item_status.show()
			# menu item: Power Source
			self.item_power = gtk.MenuItem()
			self.item_power.set_label("Power Source:")
			self.item_power.connect("activate", self.main_menu_action)
			self.item_power.show()
			# Powersave item for Power Mode
			self.item_mode_powersave.connect("toggled", self.powermode_powersave)
			# Ondemand item for Power Mode
			self.item_mode_ondemand.connect("toggled", self.powermode_ondemand)
			# Conservative item for Power Mode
			self.item_mode_conservative.connect("toggled", self.powermode_conservative)
			# Performance item for Power Mode
			self.item_mode_performance.connect("toggled", self.powermode_performance)
			### add basic items in menu
			menu.append(self.item_status)
			menu.append(item_separator)
			menu.append(self.item_power)
			menu.append(self.item_mode_powersave)
			menu.append(self.item_mode_ondemand)
			menu.append(self.item_mode_conservative)
			menu.append(self.item_mode_performance)
			menu.append(self.power_separator)
			menu.append(self.item_pmsettings)
			### show sleep actions in menu
			# menu item: ----
			self.item_sleep_separator = gtk.SeparatorMenuItem()
			self.item_sleep_separator.show()
			menu.append(self.item_sleep_separator)
			# menu item: Suspend
			self.item_suspend = gtk.MenuItem("S_uspend", True)
			self.item_suspend.connect("activate", self.suspend)
			self.item_suspend.show()
			# menu item: Hibernate
			self.item_hibernate = gtk.MenuItem("Hiber_nate", True)
			self.item_hibernate.connect("activate", self.hibernate)
			self.item_hibernate.show()
			### add sleep items in menu
			menu.append(self.item_suspend)
			menu.append(self.item_hibernate)
			### show session actions in menu
			# menu item: ----
			self.item_session_separator = gtk.SeparatorMenuItem()
			self.item_session_separator.show()
			# menu item: Log Out...
			self.item_logout = gtk.MenuItem("_Log Out...", True)
			self.item_logout.connect("activate", self.main_menu_action)
			self.item_logout.show()
			# menu item: Shut Down...
			self.item_shutdown = gtk.MenuItem("_Shut Down...", True)
			self.item_shutdown.connect("activate", self.main_menu_action)
			self.item_shutdown.show()
			# add session items in menu
			menu.append(self.item_session_separator)
			menu.append(self.item_logout)
			menu.append(self.item_shutdown)
			### set main menu
			self.indicator.set_menu(menu)
			### update status of menu items
			self.update_main_menu("sleep")
			self.update_main_menu("session")
			self.update_main_menu("power")
			self.update_main_menu("cpufreq")
		...
		def main_menu_battery_dialog(self, item = False):
			...
			# local callback for destroying battery dialog
			def dialog_destroy(arg):
				# unlock menu
			...
			# connect battery dialog to destroy event
			message_battery_status.connect("destroy", dialog_destroy)
			# show and present battery dialog
			message_battery_status.show()
			message_battery_status.present()
		
		
		def main_menu_action(self, item):
			...
			### Power Management item
				...
				os.system("gnome-power-preferences &")
			### Session items
		...
		def dbus_error(self, exception_text):
			...
			title_text = "Battery Indicator error."
			message_text = "Indicator can't get power state of computer. D-Bus system or UPower/DeviceKit-Power information unavailable now.\n"
			question_text = "Would you like still to keep indicator?"
		...
		def power_dbus_signal_error(self, error_type):
			...
			# set up icon
			self.indicator.set_icon("gpm-ac-adapter")
			# show text in main menu items and tooltip according to current error
			if error_type == "dbus":
				self.item_status.set_label("No Information")
				self.item_power.set_label("Power Source: No Information")
			elif error_type == "no_battery":
				self.item_status.set_label("No Battery")
				self.item_power.set_label("Power Source: AC Line")
			# make battery information dialog unavailable
			self.item_status.set_sensitive(False)
		
		
		def power_dbus_signal(self, device = None):
			...
			if self.lock_battery_status or self.power_error or self.no_battery:
				...
				self.item_status.set_sensitive(True)
			# get sleep actions state
			self.can_suspend = self.power_properties_interface.Get(self.power_address, "can-suspend")
			self.can_hibernate = self.power_properties_interface.Get(self.power_address, "can-hibernate")
			# update sleep menu items according its state
			if self.option_sleep and (self.can_suspend or self.can_hibernate):
				self.item_sleep_separator.show()
				if self.can_suspend:
					self.item_suspend.show()
				else:
			...
			self.status_text = ""
			if battery_time != "-:--" and self.battery_state != self.power_battery_state[self.battery_full]:
		...
		def powermode_powersave(self, item):
			"""Power settings management for Powersave CPU frequency scaling mode"""
			if self.powermode != "powersave":
				for cpu in self.cpu_items:
			...
			if cpu_switch and cpu_mode == self.powermode and self.package_power and self.option_gpm:
				self.power_settings(self.power_dev, cpu_mode)
		
		
		def powermode_ondemand(self, item):
			"""Power settings management for Ondemand CPU frequency scaling mode"""
			if self.powermode != "ondemand":
				for cpu in self.cpu_items:
		...
		def powermode_conservative(self, item):
			"""Power settings management for Conservative CPU frequency scaling mode"""
			if self.powermode != "conservative":
				for cpu in self.cpu_items:
		...
		def powermode_performance(self, item):
			"""Power settings management for Performance CPU frequency scaling mode"""
			if self.powermode != "performance":
				for cpu in self.cpu_items:
		...
		def power_management(self):
			"""Power settings management for changing power source"""
			cpu_mode = self.powermode
			power_switch = True
			...
			self.init_cpufreq()
			# if power state has changed, then switch settings, if available
			if power_switch and cpu_mode == self.powermode and self.package_power and self.option_gpm:
				self.power_settings(self.power_dev, cpu_mode)
	...
	module_indicator = False


class BatteryApplet(gnomeapplet.Applet):
	...
	def main_menu_show(self, event):
		...
		pixbuf_power = self.icon_theme.load_icon("gnome-cpu-frequency-applet", 16, gtk.ICON_LOOKUP_FORCE_SVG)
	...
	def menu_position(self, menu):
		...
		# detect applet size
		x_size, y_size = self.applet.size_request()
		# detect coordinates of window with applet
		x_origin, y_origin = self.applet.window.get_origin()
		# detect menu size
	...
	def init_gconf_settings(self):
		...
		# set up connection to GConf
		self.g_conf = gconf.client_get_default()
		# get applet related settings
		self.applet_settings = self.applet.get_preferences_key()
		# check settings
		settings = self.g_conf.dir_exists("/apps/battery_status")
		# if GConf settings not available from schema file, set up defaults
		if not settings:
	...
	def change_background(self, widget, background, color, pixmap):
		"""Change widget appearance with background panel properties"""
		# get set of widgets for changing
		widgets = (self.applet, self.event_box, self.hbox, self.icon, self.label)
		if background == gnomeapplet.PIXMAP_BACKGROUND:
			for widget in widgets:
				...
				self.option_panel_bgstate = 1
		elif background == gnomeapplet.NO_BACKGROUND:
			for widget in widgets:
				if self.option_panel_bgstate == 1:
					copy = widget.get_style().copy()
					copy.bg_pixmap[gtk.STATE_NORMAL] = pixmap
					copy.bg_pixmap[gtk.STATE_INSENSITIVE]  = pixmap
					widget.set_style(copy)
	...
	def destroy(self, event):
		"""Destroy callback (on removing applet from panel)"""
		if type(self.lock_battery_status).__name__ == "Dialog":
			# destroy battery status dialog, if active
			self.lock_battery_status.destroy()
		if self.applet_settings:
			# clean up applet related settings from GConf
			self.g_conf.remove_dir(str(self.applet_settings))
		# delete applet itself
		del self.applet
...
def run_in_panel():
	gnomeapplet.bonobo_factory("OAFIID:GNOME_BatteryApplet_Factory", BatteryApplet.__gtype__, "Battery Applet", "0", applet_factory)
	sys.exit(0)
...
def run_in_window():
	# generate gtk window for applet
	main_window = gtk.Window(gtk.WINDOW_TOPLEVEL)
	main_window.set_keep_above(True)
	main_window.set_title("Battery Status")
	main_window.set_property("resizable", False)
	main_window.set_property("window-position", "center")
	main_window.set_icon(gtk.icon_theme_get_default().load_icon("gnome-power-manager", 128, gtk.ICON_LOOKUP_FORCE_SVG))
...
def main(args):
	if len(args) == 2:
		if args[1] == "--window":
			...
			print "\t--window\t- run battery status in window (for debug and testing)"
